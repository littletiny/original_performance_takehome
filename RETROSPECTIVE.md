...
### 10.6 收尾:ramp 修复的证伪、PF2、与 980 平台

第二轮后半段做了一连串针对 ramp/drain 的实验,结论比结果更重要:

1. **诊断**:逐拍资源剖析显示 load 在 c21-38 有 ~36 槽空窗(等 gather
   地址),尾部 c960+ load 到 966 才收工。假设:"drain 是 load-bound,
   修 ramp 空窗能赚 ~18c"。
2. **三连修复**:gen_vaddr 地址生成从 +32 串行链改成 log 深度偏移树、
   prexor 30 块地址同样树化、输入 vload 提前发射 + 调度 key 加
   loadfan(距 load 后代的跳数)提升项。ramp 空窗**确实关闭了**
   (强制大提升项时 vload 完美压到 c2-27)。
3. **证伪**:但 cycles 纹丝不动(981-984),强制关闭空窗反而 990。
   因果链错了:ramp 的 load 空窗是调度器主动的负载均衡(把 load 工作
   后移填别的洞),程序总长由 **valu 吞吐(960c 下界)+ 链尾**决定,
   不是 load。教训:**资源空窗 ≠ 瓶颈;先强制关闭空窗做对照实验,
   再投入结构改造**。三处改动全部 revert。
4. **PF2(2-bit pfold)**:把对方 pfold 推广为跳过 r13 的 p-update,
   r14 用 `madd(p12,-4,aux)` + 两比特 vsel 树恢复 addr4。每组
   −1 valu +2 flow。计数如预测,但实测 PF2=12→983、PF2=20→1001:
   flow 总量逼近 955+ 后,ramp 段 flow 空闲(hash 链 ~14c 内无 select
   可做)开始支配。**flow 引擎的 ramp 地板使它不能承载过多总量**,
   这与"valu-bound 时 flow 是免费引擎"的直觉相反。
5. **平台**:最终 985 → **980**(嵌入表,finalize 5 种子机器校验 +
   9/9 测试绿)。本阶段 ~15 个配置 × 数千 basin 全部收敛 980-986。
   定量现状:valu 5760(960c 下界)+ ramp 饥饿 ~10c + 链尾 ~10c ≈ 980。
   对方 959 = valu 954 下界 + ~5c slack。**剩余 20c 差距 = 他们少 41 个
   valu op(mflex/pfold 摆放)+ slack 5 vs 21 的调度极致度**,
   两者在本框架内都未攻克;再往下需要调度器层面的质变(如逐拍
   引擎自由选择 + 更长链的代数折叠),而不是参数搜索。

### 10.7 附录:"+1 flow 版 pfold" 的推演与实测

问题:对方 pfold 每组 −1 valu +1 flow,我们的 PF2 是 +2 flow(2-bit
树),能否也做到 +1?

- **路线 1(V0−2 技巧)**:`aux = vsel(V0−2, V0, b13)`,`V0` 复用
  FOLD15 已有的 vsel。每组 −1 madd、+1 vsel、+1 减法。减法必须放 alu
  (valu 无余量),8 lane/组。实测计数:PF2=8 时 valu 959.2c、
  **alu 恰好撞 960.0c** —— 墙从 valu 平移到 alu,净下界不变。
  搜索验证(500 basin × 2 seed):981 / 983,与基线相同。已实现为
  `PF2MODE=1`,默认关闭。
- **路线 2(把 gather 边界从 d=4 提前到 d=3,真正复刻对方代数环境)**:
  对方 pfold 干净的前提是 d=3 已 gather,跳过一轮路径更新后恢复式只需
  吞 1 个 bit。我们 d=3 是 blend,窗口有两轮(r13/r14),恢复式必须吞
  2 个 bit。提前边界会让 r15 多一步地址递推(+1 madd +1 vsel +8 load),
  符号为负,未实现。
