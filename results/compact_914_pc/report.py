...
def main():
    ...
    assert sum(sum(row.values()) for row in categories.values())==accounting['weighted_alu_valu']==54484
    ...
    previous=subprocess.check_output(['git','show','2dd2238:perf_takehome.py'],cwd=ROOT,text=True)
    def outside(text):
    ...
    tests={}
    for name,expected in (('submission',9),('native',3),('regression',29)):
    ...
    report=dict(source_sha256=digest,production_changes_confined_to_generated_block=True,
                ...
                static_slot_operations=primary['static_slots'],weighted_arithmetic=54484,
                previous_914_weighted_arithmetic=54536,**tests,
                ...
                strict_sub900_counts_only_remaining_reduction=54484-60*899)
    assert report['static_bundles']==10807 and bound['bound']==910
